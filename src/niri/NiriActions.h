...
public:
    ...
    struct WorkspaceReference {
        ...
        Kind kind() const;
        // False for an index that a `u8` cannot hold or an empty name. Such a reference is refused
        // before anything is sent rather than truncated into a different workspace.
        bool isValid() const;
